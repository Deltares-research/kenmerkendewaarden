...
for current_station in stat_list:
    ...
    if compute_slotgem and data_pd_meas_all is not None:
        ...
        # remove all values after tstop_dt (is year_slotgem)
        ...
        wl_mean_peryear = slotgemiddelden_all["wl_mean_peryear"]
        wl_mean_peryear_valid = slotgemiddelden_valid["wl_mean_peryear"]
        pred_pd_wl = slotgemiddelden_valid["wl_model_fit"]
        #plot values
        if data_pd_HWLW_all is not None:
        ...
        ax1.plot(wl_mean_peryear, 'x', color='grey', label='yearly means incl invalid')
        ...
        ax1.plot(pred_pd_wl, ".-", label='model fit')
        ax1.set_prop_cycle(None) #reset matplotlib colors
        if data_pd_HWLW_all is not None:
            pred_pd_HW = slotgemiddelden_valid["HW_model_fit"]
            pred_pd_LW = slotgemiddelden_valid["LW_model_fit"]
            ax1.plot(pred_pd_HW, ".-", label=None)
            ax1.set_prop_cycle(None) #reset matplotlib colors
            ax1.plot(pred_pd_LW, ".-", label=None)
            ax1.set_prop_cycle(None) #reset matplotlib colors
        
        #2021.0 value (and future)
        slotgem_time_value = pred_pd_wl.iloc[[-1]]
        ax1.plot(slotgem_time_value, ".k", label=f'model fit for {year_slotgem}')
