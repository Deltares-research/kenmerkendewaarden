...
def test_gemiddeld_getijkromme_av_sp_np_corr(dir_meas_timeseries, dir_meas_extremes):
    ...
    assert np.isclose(prediction_av_corr["values"].max(), 1.1229166666666668) # 1.1300000000000003 in pandas<=2.2

    assert len(prediction_sp_corr) == 3701
    assert np.isclose(prediction_sp_corr["values"].min(), -0.635) # -0.5700000000000001 in pandas<=2.2
    assert np.isclose(prediction_sp_corr["values"].max(), 1.32 ) # 1.3450000000000002 in pandas<=2.2

    assert len(prediction_np_corr) == 3771
    assert np.isclose(prediction_np_corr["values"].min(), -0.61)
    assert np.isclose(prediction_np_corr["values"].max(), 0.89) # 0.8650000000000001 in pandas<=2.2
...
def test_gemiddeld_getijkromme_av_sp_np_corr_boi(dir_meas_timeseries, dir_meas_extremes):
    ...
    assert np.isclose(prediction_av_corr_boi["values"].max(),  1.1229166666666668) #1.1300000000000003)

    assert len(prediction_sp_corr_boi) == 8196
    assert np.isclose(prediction_sp_corr_boi["values"].min(), -0.635) # -0.5700000000000001)
    assert np.isclose(prediction_sp_corr_boi["values"].max(), 1.32) # 1.3450000000000002)

    assert len(prediction_np_corr_boi) == 8196
    assert np.isclose(prediction_np_corr_boi["values"].min(), -0.61)
    assert np.isclose(prediction_np_corr_boi["values"].max(), 0.89) #0.8650000000000001)
