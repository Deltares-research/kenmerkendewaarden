...
def test_calc_overschrijding(df_ext_12_2010_2014):
    ...
    expected_keys = ['Ongefilterd', 'Trendanalyse', 'Weibull', 'Gecombineerd', 'Geinterpoleerd']
    assert set(dist.keys()) == set(expected_keys)
    assert np.allclose(dist['Geinterpoleerd'].index, Tfreqs_interested)
    expected_values = np.array([1.93      , 2.09327434, 2.26311592, 2.44480348, 2.70434509,
                                2.91627091, 3.14247786, 3.46480369, 3.72735283, 4.00701551])
...
def test_calc_overschrijding_with_hydra(df_ext_12_2010_2014):
    ...
    df_hydra = pd.DataFrame({'values': np.array([2.473, 3.18 , 4.043, 4.164, 4.358, 
                             ...
                             index=np.array([1.00000000e+00, 1.00000000e-01, 2.00000000e-02, 1.00000000e-02,
...
def test_calc_overschrijding_rule_type_break(df_ext_12_2010_2014):
    Tfreqs_interested = [5, 2, 1, 1/2, 1/5, 1/10, 1/20, 1/50, 1/100, 1/200]
    dist = kw.calc_overschrijding(df_ext=df_ext_12_2010_2014, interp_freqs=Tfreqs_interested,
                                  rule_type='break', rule_value="2012")
    
    expected_keys = ['Ongefilterd', 'Trendanalyse', 'Weibull', 'Gecombineerd', 'Geinterpoleerd']
    assert set(dist.keys()) == set(expected_keys)
    assert np.allclose(dist['Geinterpoleerd'].index, Tfreqs_interested)
    expected_values = np.array([1.93      , 2.11353631, 2.37418889, 2.61405772, 2.90685818,
                                3.11376817, 3.31039598, 3.55705172, 3.73504553, 3.90661043])
    assert np.allclose(dist['Geinterpoleerd']['values'].values, expected_values)
...
def test_calc_overschrijding_clip_physical_break(df_ext_12_2010_2014):
    ...
    tstart_1931 = pd.Timestamp(1931, tstart_2010.month, tstart_2010.day, 
                               tstart_2010.hour, tstart_2010.minute, tstart_2010.second, 
                               tz=tstart_2010.tz)
    ...
    Tfreqs_interested = [5, 2, 1, 1/2, 1/5, 1/10, 1/20, 1/50, 1/100, 1/200]
    dist_normal = kw.calc_overschrijding(df_ext=df_ext_12_1931_1935, interp_freqs=Tfreqs_interested,
                                         clip_physical_break=False)
    dist_clip = kw.calc_overschrijding(df_ext=df_ext_12_1931_1935, interp_freqs=Tfreqs_interested,
                                         clip_physical_break=True)
    
    expected_values_normal = np.array([1.93      , 2.09327434, 2.26311592, 2.44480348, 2.70434509,
                                       2.91627091, 3.14247786, 3.46480369, 3.72735283, 4.00701551])
    assert np.allclose(dist_normal['Geinterpoleerd']['values'].values, expected_values_normal)
    expected_values_clip = np.array([1.93      , 2.11390828, 2.37452851, 2.61437251, 2.90714768,
                                     3.11404269, 3.3106574 , 3.5572988 , 3.73528341, 3.90683996])
    assert np.allclose(dist_clip['Geinterpoleerd']['values'].values, expected_values_clip)
